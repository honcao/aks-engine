...
func GenerateKubeConfig(properties *api.Properties, location string) (string, error) {
	...
	if properties.OrchestratorProfile != nil &&
		...
		kubeconfig = strings.Replace(kubeconfig, "{{WrapAsVerbatim \"reference(concat('Microsoft.Network/publicIPAddresses/', variables('masterPublicIPAddressName'))).dnsSettings.fqdn\"}}", api.FormatAzureProdFQDNByLocation(properties.MasterProfile.DNSPrefix, location, properties.GetCloudProfileName()), -1)
	}
	...
	if properties.AADProfile == nil {
		...
		authInfo = fmt.Sprintf("{\"auth-provider\":{\"name\":\"azure\",\"config\":{\"environment\":\"%v\",\"tenant-id\":\"%v\",\"apiserver-id\":\"%v\",\"client-id\":\"%v\"}}}",
			helpers.GetCloudTargetEnv(location, properties.GetCloudProfileName()),
			tenantID,

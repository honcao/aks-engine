...
// GetCloudProviderConfigOverwrite return the JSON format string for custom environment
func (p *Properties) GetCloudProviderConfigOverwrite(escape bool) (string, error) {

	masterProfile := p.MasterProfile
	if masterProfile == nil {
		return "", fmt.Errorf("MasterProfile cloud not be nil when GetCloudProviderConfigOverwrite")
	}

	if masterProfile.IsStandaloneKubelet == nil || !(*masterProfile.IsStandaloneKubelet) {
		return "", fmt.Errorf("IsStandaloneKubelet cloud not be nil or false when GetCloudProviderConfigOverwrite")
	}

	cppo := masterProfile.CloudProviderProfileOverride
	if cppo == nil {
		return "", fmt.Errorf("CloudProviderProfileOverride cloud not be nil when GetCloudProviderConfigOverwrite")
	}

	var cppoJSON string
	bytes, err := json.Marshal(cppo)
	if err != nil {
		return "", fmt.Errorf("Could not serialize Environment object - %s", err.Error())
	}
	cppoJSON = string(bytes)
	if escape {
		cppoJSON = strings.Replace(cppoJSON, "\"", "\\\"", -1)
	}
	return cppoJSON, nil
}

...
func (a *Apiloader) LoadContainerServiceForAgentPoolOnlyCluster(
	...
	case v20170831.APIVersion:
		...
		if managedCluster.Properties.KubernetesVersion == "" && defaultKubernetesVersion != "" {
			if !common.IsSupportedKubernetesVersion(defaultKubernetesVersion, isUpdate, hasWindows, managedCluster.Properties.GetCloudType()) {
		...
		if len(managedCluster.Properties.KubernetesVersion) > 0 && !common.IsSupportedKubernetesVersion(managedCluster.Properties.KubernetesVersion, isUpdate, hasWindows, managedCluster.Properties.GetCloudType()) {
